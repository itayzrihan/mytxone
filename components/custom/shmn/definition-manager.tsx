...
const DefinitionManager = () => {
  useEffect(() => {
    ...
    const closeModal = () => {
      ...
      const contentInput = document.getElementById('definition-content') as HTMLTextAreaElement;
    ...
    const editDefinition = async (definitionId: string) => {
      try {
        ...
        if (contentInput) contentInput.value = definition.content;
    ...
    const viewDefinition = async (definitionId: string) => {
      try {
        ...
        if (contentElement) contentElement.textContent = definition.content;
    ...
    const saveDefinition = async () => {
      ...
      if (!content) {
        alert('Please enter definition content');
        return;
      }
      ...
      try {
        ...
        // Save definition to database via API
        ...
        alert('Failed to save definition. Please try again.');
      }
    };
        ...
        definitions.forEach((definition: Definition) => {
          ...
          card.innerHTML = `
            ...
            <div class="flex justify-between items-center">
              <span class="text-text-secondary text-xs">Updated: ${updatedDate}</span>
