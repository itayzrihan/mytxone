...
const DefinitionManager = () => {
  useEffect(() => {
      ...
      try {
        ...
        if (contentElement) {
          ...
          try {
            ...
            const formattedContent = entries.map((entry: any) => 
    ...
    const saveDefinition = async () => {
      ...
      let combinedContent = '';
      if (definitionContentContainer) {
        const contentEntries = definitionContentContainer.querySelectorAll('.entry-content') as NodeListOf<HTMLTextAreaElement>;
        
        // Convert entries to array of objects
        const entriesArray = Array.from(contentEntries).map((entryTextarea, index) => {
          return {
            index: index + 1,
    ...
    if (editViewDefinitionBtn) {
      editViewDefinitionBtn.addEventListener('click', (e: Event) => {
        const target = e.target as HTMLElement;
        const definitionId = target.getAttribute('data-definition-id');
        if (definitionId) {
          editDefinition(definitionId);
    ...
    return () => {
      ...
      if (editViewDefinitionBtn) {
        editViewDefinitionBtn.removeEventListener('click', (e: Event) => {
          const target = e.target as HTMLElement;
          const definitionId = target.getAttribute('data-definition-id');
          if (definitionId) {
            editDefinition(definitionId);
