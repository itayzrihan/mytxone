...
const ProtocolManager = () => {
  useEffect(() => {
    ...
    const addProtocolPart = (partNumber: number) => {
    ...
    const saveProtocol = async () => {
      ...
      try {
        ...
        if (consoleContent) {
          ...
          message.innerHTML = `<span class="text-green-500">[${new Date().toLocaleTimeString()}]</span> Protocol <strong>${name}</strong> created successfully with ${parts.length} parts.`;
