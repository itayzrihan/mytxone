      ...
      {memories.length > 0 && (
        <CardContent>
          <ul className="space-y-3">
            {memories.map((memory) => (
              <li key={memory.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-2">
                <span className="grow mb-1 sm:mb-0 mr-2">{memory.content}</span>
          ...
          {!multipleFound && memories.length === 1 && (
            <p className="text-xs text-muted-foreground mt-3">To forget this, reply &quot;yes&quot; or tell me: &quot;Forget memory {memories[0].id}&quot;.</p>
          )}
        </CardContent>
      )}
    </Card>
