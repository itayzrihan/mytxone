...
export function CustomAnimationControls({ settings, onSettingsChange }: CustomAnimationControlsProps) {
  ...
  const changeOrientation = (newOrientation: TemplateOrientation) => {
    // Detect which template is currently loaded by checking the content
    let currentTemplate = 'climbing'; // default
    if (settings.unifiedCode.includes('Shape Morphing') || settings.unifiedCode.includes('Constant Evolution')) {
      currentTemplate = 'morphing';
    } else if (settings.unifiedCode.includes('Pure Energy') || settings.unifiedCode.includes('Unleashing unlimited potential')) {
      currentTemplate = 'energy';
    } else if (settings.unifiedCode.includes('Reaching New Heights') || settings.unifiedCode.includes('Every step forward')) {
      currentTemplate = 'climbing';
    }

    // Update orientation and reload the template with new orientation
    updateSettings({ orientation: newOrientation });
    
    // Use setTimeout to ensure state is updated before reloading template
    setTimeout(() => {
      const templateCode = getTemplateByName(currentTemplate, newOrientation);
      updateSettings({ orientation: newOrientation, unifiedCode: templateCode });
    }, 50);
  };
